...
7. 损失函数
     ...
     * 框回归损失采用GIoU损失函数，仅当预测框内包含物体时计算；
       ...
       - 解释一下这里计算bbox 坐标回归的时候为什么调制系数为（2-wh），wh为归一化的真实标注bbox的长宽   
          理解1：w*h的取值范围为(0,1],当整个图为一个bbox的时候w*h最大=1，所以2-1就回归不加调制的方式，目标较小的时候，调制权重比较大，也是让模型更加聚焦到小目标的坐标回归当中。但是不能为1-wh，这样的话目标越大，得数越接近于0，失去了调制效果。那为什么不能是3，5，甚至10呢？个人觉得3的话也是可以，只不过无形之中增加了坐标回归Loss（coor）在整体loss中的占比，和整体Loss中的Lambda（pos）就重复了，所以设置为2最合适   
          理解2：2-wh可以写为（1+1-wh），前面第一个『1』代表本来自然的无调制时候的坐标回归loss，后面的1-wh相当于在大小目标上做调制，大目标的得分小，降低权重，小目标的（1-wh）相比大目标大，所以思想主要还是调节大小目标的侧重不同    
          所以针对不同数据集，可以根据自己数据中的大小目标占比来适当的调节这个系数，也不是完全按照论文中的系数来，灵活调整。
